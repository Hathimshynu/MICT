...
.ai-on-off-switch .form-switch .form-check-input {
     background-image: url("data:image/svg+xml,%3csvg width='20' height='20' viewBox='0 0 20 20' xmlns='http://www.w3.org/2000/svg'%3e%3crect x='1' y='1' width='18' height='18' rx='9' fill='%23FFFFFF' stroke='%23000000' stroke-width='2'/%3e%3c/svg%3e") !important;
}
    ...
    border-color: #ff0000 !important;
      ...
      <div class="row">
          <div class="col-lg-12">
             ...
             <div style="padding-bottom: 22px !important;" class="card-header d-block d-sm-flex d-md-flex d-lg-flex d-xl-flex d-xxl-flex justify-content-between">
                    <h4 class="card-title trading-heading text-center mb-0">AI Trading </h4>
           ...
           ?>
                 ...
                 <div class="card-header d-flex justify-content-between">
                    <h4 class="card-title mb-0">AI Trading Investment</h4>
                    <!--<p><a href='<?=BASEURL?>user/ai_trading_transfer_history'><button class="btn btn-primary btn-sm">AI Trading Transfer History</button></a></p>-->
      ...
      <!--<div class="col-lg-6 col-md-12 col-sm-12 col-xl-6">-->
						...
						<!--			</div>-->
								...
								<?php $balance =  $this->db->select("(SUM(credit) - SUM(debit)) as balance")->where('user_id', $this->session->userdata('micusername'))->get('e_wallet')->row()->balance + 0;?>
														...
														<h5 class="mb-2 number-font text-primary"><?=sprintf('%.2f',floatval($balance));?> USDT</h5><a href="<?=BASEURL?>user/ai_trading_transfer_history"><span>Transfer History</span></a>
										...
										<div class="card shining-card wallet overflow-hidden">
											<div class="card-body">
												<div class="row">
														...
														<h4 class="" style="font-size:18px">AI Trading Wallet <i class="bi bi-arrow-clockwise"></i></h4>
														<h5 class="mb-2 number-font text-primary"><?=$tradbal;?> USDT</h5><a href="<?=BASEURL?>user/portfolio"><span>View Portfolio</span></a>
        ...
        <form id="deposit">
                  <div class="card-body">
                    ...
                    <label class="form-label">Select Slab</label>
                      <?php
                    // $isFriday = date('N') == 5; // Check if today is Friday (1 for Monday, 2 for Tuesday, ..., 7 for Sunday)
                    ?>
                  ...
                  <div class="form-group">
                    ...
                    <select name='package' id="packageSelect" class="form-select mb-3 shadow-none">
                     ...
                     <div class="text-center">
                                ...
                                // Query to check for entries with today's date and matching remark and user_id
                                $entryQuery = $this->db->select('entry_date')
                                    ->where('remark', 'Transfer To Trading Wallet')
                                    ->where('user_id', $this->session->userdata('micusername'))
                                    ->where('DATE(entry_date)', $today)
                                    ->get('trading_wallet');
                                
                                // Check if there are entries for today
                                $isDisabled = ($entryQuery->num_rows() > 0);
                                ?>
                         <button type="submit" class="btn btn-success  mt-4 mb-0" fdprocessedid="9a036e"><i class="fa-regular fa-circle-check me-2"  ></i>Invest</button>
                         </div>
                     </form>                  </div>
               </div>
            </div>

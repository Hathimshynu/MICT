...
<?php include 'header.php';?>
      ...
      <div class="row">
        ...
        <tbody>
   <?php $binary_income=$this->db->where('remark','Pair Income')->where('credit >',0)->where('pair_id !=','')->where('username',$this->session->userdata('micusername'))->where('entry_date <=',date('Y-m-d H:i:s'))->get('account')->result_array();
         $count=1;
         foreach($binary_income as $key=>$binary){ 
         $income=$this->db->select('sum(credit) + sum(ewallet) as balance')->where('description',$binary['pair_id'])->get('account')->row()->balance+0; ?>
            <tr>
                <td><?=$count++?></td>

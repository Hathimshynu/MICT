...
class Dist extends CI_Controller {
	...
	public function update_profile()
		{
			...
			if($_POST){
    			...
    			if($this->form_validation->run()==true) {
    				...
    				$profile = $this->db->insert('profile_request',$data);
    				if ($profile){
	...
	public function search_userid()
		{
			...
			$check = $this->db->get('user_role')->row_array();
			if (!empty($check)) {
				redirect('genealogy/'.$check['user_role_id'],'refresh');
			} else {
	...
	public function password_reset()
	{
		if ($_POST) {
			...
			if ($this->db->update('user_role',array('pwd'=>$new_pass))) {
	...
	public function checkutr(){
		if($this->db->where('utr',$this->input->post('utr'))->count_all_results('admin_request')+0 > 0){
			$this->form_validation->set_message('checkutr', 'is already Exits');
			return false;
		} else {
			return true;
		}
	}
	public function admin_request_auto()
	{
		if ($_POST) {
	...
	public function formcheck($data, $datafield){
		if($this->db->where($datafield,$data)->count_all_results('profile') > 7){
	...
	public function forgot_password()
	{
		...
		public function withdraw()
		{
			...
			if ($_POST) {
				...
				if ($pin_value != 0) {
					if ($this->admin->add_withdraw_request($user_id)) {
						$this->session->set_flashdata('success_message', 'Request Successfully');
		...
		public function gen_roi(){
			$roi_date = date('Y-m-d', strtotime("yesterday"));
			$payouts = $this->db->where('roi_date <',$roi_date)->get('roi')->result_array();
			foreach ($payouts as $key => $n) {
				$ceilings = ($this->db->select_sum('wallet_value')->where('status','Accepted')->where('user_id',$n['user_id'])->get('admin_request')->row()->wallet_value+0)*2; 
				$totalincome = $this->db->select_sum('credit')->where('user_id',$n['user_id'])->get('account')->row()->credit+0;
				$celing = $ceilings-$totalincome;
				$roiamount = min($celing,$n['amount']);
				$dataearn1 = array(
					'user_id' => $n['user_id'],
					'entry_date'=> $n['roi_date'],
					'credit' => $roiamount,
					'balance' => $roiamount+($this->db->order_by('account_id',"DESC")->limit(1)->where('user_id',$n['user_id'])->get('account')->row()->balance+0),
					'remark' => 'ROI',
		...
		public function view_kyc()
		{
			...
			if($_POST){
				if($this->input->post('datastatus') == "Reject"){
					...
					$data_b['replay'] = $this->input->post('replay');
		...
		public function view_profile()
		{
			if ($this->session->userdata('login') != "1") 
				redirect('','refresh');
			$data['users'] = $this->db->order_by('profile_request_id','DESC')->get('profile_request')->result_array();
			...
			if($_POST){
				if($this->input->post('datastatus') == "Reject"){
					...
					$re_ban = $this->db->update('profile_request',$data_b);
					if ($re_ban == true) {
						$this->session->set_flashdata('success_message', 'Request Rejected Successfully');
						redirect('view_profile','refresh');  
					} else {
						$this->session->set_flashdata('Error_message', 'Not Successfully');
						redirect('view_profile','refresh');  
					}
		...
		public function bankcheck($data, $datafield){
			if($this->db->where($datafield,$data)->where('user_id !=',$this->session->userdata('userid'))->count_all_results('bank')+0 > 0){
		...
		public function bank()
		{
			...
			if($_POST){
				...
				if($this->form_validation->run()==true) {
					...
					$this->session->set_flashdata('error_message', 'Account number already Exits');
					redirect('bank','refresh');
				}
			}
            $data['bank_request'] = $this->db->get_where('bank_request',array('user_id'=>$this->session->userdata('userid')))->result_array();
		...
		public function kyccheck($data, $datafield){
			if($this->db->where($datafield,$data)->where('user_id !=',$this->session->userdata('userid'))->count_all_results('kyc')+0 > 0){
		...
		public function kyc()
		{
			...
			if($_POST){
				...
				if($this->form_validation->run()==true) {
					...
					$this->session->set_flashdata('error_message', 'Pan number / Aadhar number already Exits');
					redirect('kyc','refresh');
				}
			}
			$data['cust'] = $this->admin->get_row_data('kyc','user_id',$this->session->userdata('userid'));
			$data['kyc_request'] = $this->db->get_where('kyc_request',array('user_id'=>$this->session->userdata('userid')))->result_array();
			$data['page_name'] = "kyc";
	...
	//// update && edit user profiles
		public function user_profile()
		{
			...
			$data['profile_request'] = $this->db->get_where('profile_request',array('user_id'=>$this->session->userdata('userid')))->result_array();
			$data['page_name'] = "user_profile";
			$this->load->view('user_profile',$data);
		}
		...
		public function product_delivery() {
			if ($this->session->userdata('login') != "1") 
				redirect('','refresh');
			log_message("error",$this->input->post('id'));
		...
		public function view_investment_request($param1='',$param2='')
		{
			if ($this->session->userdata('login') != "1") 
				redirect('','refresh');
			$data['wallet_request'] = $this->db->order_by('status',"DESC")->get('admin_request')->result_array();
	...
	{ 
		if ($this->session->userdata('login') == 1)
		{
			if($this->session->userdata('login') == 'New'){
			} else {
